...
function gc_render_clientes_shortcode(): string {
    ...
    $edit_id = absint($_GET['edit_cliente'] ?? 0);
    $edit_data = array(
        'nombre' => '',
        'ruc' => '',
        'telefono' => '',
    ...
    $form .= wp_nonce_field('gc_save_cliente', '_wpnonce', true, false);
    $form .= '<input type="hidden" name="action" value="gc_save_cliente">';
    if ($edit_id) {
        $form .= '<input type="hidden" name="cliente_id" value="' . esc_attr($edit_id) . '">';
    }
    $form .= '<div class="gc-form-grid">'
        ...
        . '<button class="gc-button" type="submit">' . ($edit_id ? 'Actualizar cliente' : 'Agregar cliente') . '</button>'
        . '</form>';
    ...
    foreach ($rows as $row) {
        $rows_html .= '<tr>'
            ...
            . '<a class="gc-link" href="' . esc_url(add_query_arg('edit_cliente', $row['id'])) . '">Editar</a>'
            . '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '" onsubmit="return confirm(\'¿Eliminar cliente?\')">'
            . wp_nonce_field('gc_delete_cliente', '_wpnonce', true, false)
            . '<input type="hidden" name="action" value="gc_delete_cliente">'
            . '<input type="hidden" name="cliente_id" value="' . esc_attr($row['id']) . '">'
    ...
    if (!$rows_html) {
        $rows_html = '<tr><td colspan="6" class="gc-table-empty">Sin clientes cargados.</td></tr>';
    }
    ...
    return '<div class="gc-app">' . gc_wrap_panel('Clientes', 'Gestiona los datos básicos de clientes.', $panel_content) . '</div>';
}
