...
from services.editor_layout_contracts import (
...
def _sheet_area(layout: Dict) -> tuple[float, float, float, float, float, float]:
    return step_repeat_pro_engine.sheet_area(layout)


def _design_dimensions(design: Dict, layout: Dict) -> tuple[float, float, float]:
    return step_repeat_pro_engine.design_dimensions(design, layout)


def _ordered_repeat_designs(layout: Dict) -> List[Dict]:
    return step_repeat_pro_engine.ordered_repeat_designs(layout)


def _design_repeat_zone(design: Dict) -> str:
    return step_repeat_pro_engine.design_repeat_zone(design)


def _group_designs_by_zone(designs: List[Dict]) -> Dict[str, List[Dict]]:
    return step_repeat_pro_engine.group_designs_by_zone(designs)


def _get_zone_bounds(layout: Dict, zone: str) -> tuple[float, float, float, float]:
    return step_repeat_pro_engine.get_zone_bounds(layout, zone)


def _slot_overlaps_existing(candidate: Dict, existing_slots: List[Dict]) -> bool:
    return step_repeat_pro_engine.slot_overlaps_existing(candidate, existing_slots)
...
def _repeat_requested_vs_placed(
    ...
    return step_repeat_pro_engine.repeat_requested_vs_placed(designs, slots, placement_attempts)
...
def _repeat_capacity(
    ...
    return step_repeat_pro_engine.repeat_capacity(slot_w, slot_h, usable_w, usable_h, gap_x, gap_y)
...
def _choose_repeat_orientation(
    ...
    return step_repeat_pro_engine.choose_repeat_orientation(
        piece_w,
        piece_h,
        forms,
        allow_rotation,
...
def _append_step_repeat_slots_in_bounds(
    ...
    return step_repeat_pro_engine.append_step_repeat_slots_in_bounds(
        slots,
        designs,
        layout,
        bounds,
...
def _candidate_positions_for_fill(
    ...
    return step_repeat_pro_engine.candidate_positions_for_fill(bounds, slot_w, slot_h, gap_x, gap_y)
...
def _estimate_repeat_group_height(
    ...
    return step_repeat_pro_engine.estimate_repeat_group_height(designs, layout, usable_w, usable_h, gap_x, gap_y)
...
def _expanded_vertical_zone_bounds(
    ...
    return step_repeat_pro_engine.expanded_vertical_zone_bounds(layout, zone_groups, gap_x, gap_y)
...
def _append_fill_slots_smart(
    ...
    return step_repeat_pro_engine.append_fill_slots_smart(
        slots,
        designs,
        layout,
        bounds,
...
def _slot_group_bbox(slots: List[Dict], start: int, end: int) -> tuple[float, float, float, float] | None:
    return step_repeat_pro_engine.slot_group_bbox(slots, start, end)


def _translated_group_slots(slots: List[Dict], start: int, end: int, dx: float, dy: float) -> List[Dict]:
    return step_repeat_pro_engine.translated_group_slots(slots, start, end, dx, dy)
...
def _can_place_translated_group(
    ...
    return step_repeat_pro_engine.can_place_translated_group(slots, start, end, translated_slots, usable_bounds)
...
def _compact_vertical_zone_groups(
    ...
    return step_repeat_pro_engine.compact_vertical_zone_groups(slots, group_ranges, usable_bounds, min_gap_y)
...
def _translated_groups_are_safe(
    ...
    return step_repeat_pro_engine.translated_groups_are_safe(slots, translated_groups, usable_bounds)
...
def _compact_vertical_zonal_and_auto_groups(
    ...
    return step_repeat_pro_engine.compact_vertical_zonal_and_auto_groups(slots, group_ranges, usable_bounds, min_gap_y)
...
def obtener_layout_job(job_id: str):
    ...
    try:
        ...
        data = ensure_post_editor_layout_defaults(data)
    except Exception:
        abort(404)
    return jsonify(data)
...
def editor():
    ...
    layout_payload = ensure_post_editor_layout_defaults(layout_payload)
...
def _sanitize_layout_items(job_id: str, job_dir: str, meta: Dict, items: List[Dict]):
    return sanitize_post_editor_layout_items(
        job_dir=job_dir,
        meta=meta,
        items=items,
    )
